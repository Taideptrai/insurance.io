...
describe('CreateProject', () => {
  ...
  it(`User can create project with valid information(Role=Seller's Principal Advisor)`, () => {
    var d = new Date()
    var datestring =
      d.getFullYear() +
      '/' +
    ...
    //Select Buyer role
    cy.get('[role="radiogroup"]').find('[class="v-radio theme--light"]').eq(2).click()
    ...
    //Enter buyer's email
    cy.get('[name="buyerMail"]').type('buyerMail@smartdev.com')

    //Enter my support advisor's name
    cy.get('[class="padvisor"]').find('[name="principalAdvisor_name"]').eq(0).type('Support Advisor')
    //Enter my support advisor's email textfield should be displayed
    cy.get('[class="padvisor"]').find('[name="principalAdvisor_email"]').eq(0).type('support@email.com')
    ...
    cy.get('p').should('be.visible').should('contain', 'has been successfully created').should('contain', datestring)
